...
use App\Filament\Resources\AnonymousPostResource\Pages;


class AnonymousPostResource extends Resource
{
    protected static ?string $model = AnonymousPost::class;
                ...
                Card::make()->schema([
                    Grid::make()->schema([
                        ...
                        Card::make()->schema([
                            ...
                            Select::make('post_position')->label(__('Post Position'))
                                ->required()
                                ->options(Pharmalys::positions())
                                ->columnSpan(1),
                            TextInput::make('link')->placeholder('Block link')
                                ->required()
                                ->columnSpan(1),
                            FileUpload::make('image')->image()->maxSize(500)->required(),
                        ])->columnSpan(1),
    ...
    public static function getEloquentQuery(): Builder
    {
        ...
        return static::getModel()::query()->withoutGlobalScopes()->where('author_id', '!=', '');
    }
    ...
    {
        return [
            'index' => Pages\ListAnonymousPosts::route('/'),
            'create' => Pages\CreateAnonymousPost::route('/create'),
            'edit' => Pages\EditAnonymousPost::route('/{record}/edit'),
        ];
    }
}
